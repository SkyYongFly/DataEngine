...
layui.define(["element", "jquery"], function (exports) {
    ...
    frame = new function () {
        ...
        this.init = function (url) {
            ...
            $.getJSON(url, function (data, status) {
                if (data == null) {
                    frame.msg_error('暂无菜单信息');
                } else {
                    frame.initHome(data.homeInfo);
                    frame.initLogo(data.logoInfo);
                    frame.initClear(data.clearInfo);
                    frame.initMenu(data.menuInfo);
                    frame.initTab();
                }
            }).fail(function () {
                frame.msg_error('菜单接口有误');
            });
            layer.close(loading);
        };
        ...
        this.initDevice = function () {
            if (frame.checkMobile()) {
                $('.frame-tool i').attr('data-side-fold', 0);
                $('.frame-tool i').attr('class', 'fa fa-indent');
                $('.layui-layout-body').attr('class', 'layui-layout-body frame-mini');
            }
        };
        ...
        this.initHome = function (data) {
            sessionStorage.setItem('frameHomeHref', data.href);
            $('#frameHomeTabId').html('<i class="' + data.icon + '"></i> <span>' + data.title + '</span>');
            $('#frameHomeTabId').attr('lay-id', data.href);
            $('#frameHomeTabIframe').html('<iframe width="100%" height="100%" frameborder="0"  src="' + data.href + '"></iframe>');
        };
        ...
        this.initClear = function (data) {
            $('.frame-clear').attr('data-href', data.clearUrl);
        };
        ...
        this.initBgColor = function () {
            var bgcolorId = sessionStorage.getItem('frameBgcolorId');
            if (bgcolorId == null || bgcolorId == undefined || bgcolorId == '') {
                bgcolorId = frame.config('BgColorDefault');
            }
            var bgcolorData = frame.bgColorConfig(bgcolorId);
            var styleHtml = '.layui-layout-admin .layui-header{background-color:' + bgcolorData.headerRight + '!important;}\n' +
                '.layui-header>ul>.layui-nav-item.layui-this,.frame-tool i:hover{background-color:' + bgcolorData.headerRightThis + '!important;}\n' +
        ...
        this.initTab = function () {
            ...
            if (urlArr.length >= 2) {
                ...
                // 判断链接是否有效
                var checkUrl = frame.checkUrl(href);
                if (checkUrl != true) {
                    return frame.msg_error(checkUrl);
                }

                // 判断tab是否存在
                var checkTab = frame.checkTab(href);
                if (!checkTab) {
                    var title = href,
                        tabId = href;
                    $("[data-tab]").each(function () {
                        ...
                        // 判断是否带参数了
                        if (frame.config('urlSuffixDefault')) {
                    ...
                    var frameHomeTab = $('#frameHomeTab').attr('lay-id'),
                        frameHomeHref = sessionStorage.getItem('frameHomeHref');

                    // 非菜单打开的tab窗口
                    if (href == title) {
                        var frameTabInfo = JSON.parse(sessionStorage.getItem("frameTabInfo"));
                        if (frameTabInfo != null) {
                            var check = frameTabInfo[tabId];
                    ...
                    if (frameHomeTab != href && frameHomeHref != href) {
                        frame.addTab(tabId, href, title, true);
                        frame.changeTab(tabId);
                    }
                }
            }
            if (frame.config('urlHashLocation')) {
                frame.hashTab();
            }
        };
        ...
        this.buildBgColorHtml = function () {
            ...
            var bgColorConfig = frame.bgColorConfig();
        ...
        this.addTab = function (tabId, href, title, addSession) {
            if (addSession == undefined || addSession == true) {
                ...
                frameTabInfo[tabId] = {href: href, title: title}
                sessionStorage.setItem("frameTabInfo", JSON.stringify(frameTabInfo));
            }
        ...
        this.delTab = function (tabId, isParent) {
            var frameTabInfo = JSON.parse(sessionStorage.getItem("frameTabInfo"));
            if (frameTabInfo != null) {
                delete frameTabInfo[tabId];
                sessionStorage.setItem("frameTabInfo", JSON.stringify(frameTabInfo))
            }
            if (isParent === true) {
                parent.layui.element.tabDelete('frameTab', tabId);
            } else {
                element.tabDelete('frameTab', tabId);
            }
        };
        ...
        this.changeTab = function (tabId) {
            element.tabChange('frameTab', tabId);
        };
        ...
        this.hashTab = function () {
            ...
            element.tabChange('frameTab', layId);
            element.on('tab(frameTab)', function (elem) {
        ...
        this.checkUrl = function (url) {
            if (!frame.config('checkUrlDefault')) {
                return true;
            }
            var msg = true;
            $.ajax({
    ...
    $('body').on('click', '[data-tab]', function () {
        ...
        // 判断该窗口是否已经打开过
        var checkTab = frame.checkTab(tabId);
        if (!checkTab) {
            frame.addTab(tabId, href, title, true);
        }
        element.tabChange('frameTab', tabId);
    ...
    $('body').on('click', '[data-iframe-tab]', function () {
        ...
        // 判断该窗口是否已经打开过
        var checkTab = frame.checkTab(tabId, true);
        if (!checkTab) {
            var frameTabInfo = JSON.parse(sessionStorage.getItem("frameTabInfo"));
            if (frameTabInfo == null) {
                frameTabInfo = {};
            ...
            frameTabInfo[tabId] = {href: href, title: title}
            sessionStorage.setItem("frameTabInfo", JSON.stringify(frameTabInfo));
            parent.layui.element.tabAdd('frameTab', {
                title: title + '<i data-tab-close="" class="layui-icon layui-unselect layui-tab-close">ဆ</i>' //用于演示
                , content: '<iframe width="100%" height="100%" frameborder="0"  src="' + href + '"></iframe>'
                , id: tabId
            });
        }
        parent.layui.element.tabChange('frameTab', tabId);
        frame.tabRoll();
        parent.layer.close(loading);
    });
    ...
    $('body').on('click', '[data-clear]', function () {
        ...
        if (clearUrl != undefined && clearUrl != '' && clearUrl != null) {
            $.getJSON(clearUrl, function (data, status) {
                layer.close(loading);
                if (data.code != 1) {
                    return frame.msg_error(data.msg);
                } else {
                    return frame.msg_success(data.msg);
                }
            }).fail(function () {
                layer.close(loading);
                return frame.msg_error('清理缓存接口有误');
            });
        } else {
            layer.close(loading);
            return frame.msg_success('清除缓存成功');
        }
    });
    ...
    $('body').on('click', '[data-side-fold]', function () {
        ...
        if (isShow == 1) { // 缩放
            ...
            $('.frame-tool i').attr('class', 'fa fa-outdent');
            $('.layui-layout-body').attr('class', 'layui-layout-body frame-all');
        }
        frame.tabRoll();
        element.init();
        layer.close(loading);
    ...
    $('body').on('click', '[data-bgcolor]', function () {
        ...
        var bgColorHtml = frame.buildBgColorHtml();
        var html = '<div class="frame-color">\n' +
        ...
        layer.open({
            ...
            content: html,
                ...
                $('.frame-select-bgcolor').removeClass('layui-this');
            }
        });
        layer.close(loading);
    });
    ...
    $('body').on('click', '[data-select-bgcolor]', function () {
        ...
        $('.frame-color .color-content ul .layui-this').attr('class', '');
        $(this).attr('class', 'layui-this');
        sessionStorage.setItem('frameBgcolorId', bgcolorId);
        frame.initBgColor();
    });
